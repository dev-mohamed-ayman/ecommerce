...
@section('title')
    {{$settings->site_name}} || {{ __('word.about') }}
@endsection
    ...
    <section id="wsus__breadcrumb">
        <div class="wsus_breadcrumb_overlay">
            <div class="container">
                <div class="row">
                    <div class="col-12">
                        <h4>{{ __('word.blog_details') }}</h4>
                        <ul>
                            <li><a href="#">{{ __('word.blog') }}</a></li>
                            <li><a href="#">{{ __('word.blog_details') }}</a></li>
                        </ul>
                    </div>
                </div>
            </div>
                ...
                <div class="col-xxl-9 col-xl-8 col-lg-8">
                    <div class="wsus__main_blog">
                        <div class="wsus__main_blog_img">
                            <img src="{{asset($blog->image)}}" alt="{{ __('word.blog_image') }}"
                                 class="img-fluid w-100">
                        </div>
                        <p class="wsus__main_blog_header">
                            <span><i class="fas fa-user-tie"></i> {{ __('word.by') }} {{$blog->user->name}}</span>
                            <span><i
                                    class="fal fa-calendar-alt"></i> {{date('M d, Y', strtotime($blog->created_at))}}</span>
                        </p>
                                            ...
                                            <div class="wsus__single_blog wsus__single_blog_2">
                                                <a class="wsus__blog_img"
                                                   href="{{route('blog-details', $blogItem->slug)}}">
                                                    <img src="{{asset($blogItem->image)}}" alt="{{ __('word.blog') }}"
                                                         class="img-fluid w-100">
                                                </a>
                        ...
                        <div class="wsus__comment_area">
                            <h4>{{ __('word.comments') }} <span>{{count($comments)}}</span></h4>
                            @foreach ($comments as $comment)
                                <div class="wsus__main_comment">
                                    <div class="wsus__comment_img">
                                        <img style="width: 80px;height: 80px;object-fit: contain;"
                                             src="{{asset($comment->user->image)}}" alt="{{ __('word.user') }}"
                                             class="img-fluid w-100">
                                    </div>
                                    ...
                                    <div class="row">
                                        <div class="col-xl-12">
                                            <div class="wsus__single_com">
                                                <textarea rows="5" placeholder="{{ __('word.your_comment') }}"
                                                          name="comment"></textarea>
                                                <input type="hidden" name="blog_id" value="{{$blog->id}}">
                                            </div>
                                        </div>
                                    </div>
                                    <button class="common_btn" type="submit">{{ __('word.post_comment') }}</button>
                                </form>
                            @else
                                <p>{{ __('word.login_to_comment') }}</p>
                                <a class="common_btn" href="{{route('login')}}">{{ __('word.login') }}</a>
                            @endif
                        </div>
                    </div>
                </div>
